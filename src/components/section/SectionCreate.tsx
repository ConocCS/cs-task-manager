...
export function SectionCreate({ onSubmit }: SectionCreateProps) {
  ...
  if (!isEditing) {
    return (
      <button
        ...
        className="flex items-center gap-2 px-8 py-3 text-sm text-[var(--color-primary)]/60 hover:text-[var(--color-primary)] w-full border-t border-[var(--color-border)]/50 transition-colors font-medium"
      >
        <Plus size={16} />
        セクションを追加
      </button>
  ...
  return (
    <div className="px-8 py-3 border-t border-[var(--color-border)]/50">
      <input
        autoFocus
        type="text"
        value={name}
